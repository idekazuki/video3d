#This code was added to gulpio/transfomrs.py
...
class RandomCornerCrop(object):
    #This code I added.
    #This function can be cut at random from the four corners and the center. The cut size can also be random.

    def __init__(self, size=[1.0]):
        #ex) size=[1.0, 0.5] ,size=[224, 112]
        self.size = size
        self.rand_pos = random.randrange(5)
    ...
    def __call__(self, imgs):
        ...
        """
        self.rand_pos = random.randrange(5)
        print("rand_pos{}".format(self.rand_pos))
        h, w = imgs[0].shape[0], imgs[0].shape[1]
        
        s = random.choice(self.size)
        if s <= 1.0:
            s = s * h
        size = (int(s), int(s))
        ...
        if self.rand_pos == 0:
            ...
            x1 = int(round((w - tw) / 2.))
            y1 = int(round((h - th) / 2.))
        elif self.rand_pos == 1:
            #left up crop
            x1 = 0
            y1 = 0
        elif self.rand_pos == 2:
            #left down crop
            x1 = 0
            y1 = h - th
        elif self.rand_pos == 3:
            #right up crop
            x1 = w - tw
            y1 = 0
        elif self.rand_pos == 4:
            #right down crop
            x1 = w - tw
            y1 = h - th
